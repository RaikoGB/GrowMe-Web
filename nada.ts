...
//     const createUser = async (email: string , password: string , username : string , gender : number ): Promise<any> => {

//       console.log(typeof email , typeof password , typeof username , typeof gender,  'Desde auth')

//       try {
//         const resp = await growApi.post('/users/register', { email , password , username , gender });
//         console.log(resp)
//         localStorage.setItem('token', resp.data.token);
//         localStorage.setItem('token-init-date', new Date().getTime().toString());
//         dispatch(onLogin({ name: resp.data.name, id: resp.data.id }));
//       } catch (error) {
//         dispatch(onLogout('mal'));
//         setTimeout(() => {
//           dispatch( clearErrorMessage() );
//       }, 10);
...
// //     try {
// //       const resp = await growApi.post('/users/registerUser', { email, password });
// //       console.log(resp)
// //       localStorage.setItem('token', resp.data.token);
// //       //   localStorage.setItem('token-init-date', new Date().getTime().toString());
// //       dispatch(onLogin({ name: resp.data.name, id: resp.data.id }));
